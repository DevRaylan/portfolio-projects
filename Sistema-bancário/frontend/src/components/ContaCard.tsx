  ...
  return (
    <div className="rounded-lg border border-gray-200 p-4 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button type="button" onClick={() => setAcao('depositar')} className="rounded bg-green-50 px-2 py-1 text-xs font-medium text-green-700 hover:bg-green-100">
          Depositar
        </button>
        <button type="button" onClick={() => setAcao('sacar')} className="rounded bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700 hover:bg-amber-100">
          Sacar
        </button>
        <button type="button" onClick={() => setAcao('transferir')} className="rounded bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 hover:bg-blue-100">
          Transferir
        </button>
        <button type="button" onClick={() => setMostrarExtrato((v) => !v)} className="rounded bg-gray-100 px-2 py-1 text-xs font-medium text-gray-700 hover:bg-gray-200">
          {mostrarExtrato ? 'Ocultar extrato' : 'Ver extrato'}
        </button>
      </div>

      {acao && (
        <form onSubmit={handleSubmit} className="mt-3 flex flex-col gap-2 border-t border-gray-100 pt-3">
          {acao === 'transferir' && (
            <input
              type="number"
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={fecharAcao} className="rounded px-3 py-1 text-sm text-gray-500 hover:bg-gray-100">
              Cancelar
            </button>
          </div>
          {mutacaoAtiva?.isError && (
            <p className="text-sm text-red-600">{(mutacaoAtiva.error as Error).message}</p>
          )}
        </form>
      )}
            ...
            <ul className="flex flex-col gap-1">
              {conta.historico.map((transacao, index) => (
                <li key={index} className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">{ROTULO_TIPO[transacao.tipo] ?? transacao.tipo}</span>
