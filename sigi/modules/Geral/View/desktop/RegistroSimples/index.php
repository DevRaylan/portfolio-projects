<?php 
    ...
    if ($tipoRegistro == 'Funcao') $tipoRegistro = 'Função';
?>
    ...
    <table id="simple-record-table" class="table table-striped table-hover table-bordered tabela-servicos dt-responsive">
        <thead>
            <tr>
                ...
                <?php  if ($tipo == 'setor') echo "<th class='col-sm-1'>Sigla</th>"; ?>
        ...
        <form class="form-horizontal" method="post" id="form-importar">
            ...
            <div class="form-group" id="msg_file">
                <label class="col-sm-6 control-label">
                    Atenção: o arquivo deve ser do tipo .csv sem cabeçalho.
                    <?php if ($tipo == 'setor') echo "<br>A sigla deve ser separada da descrição por vírgula, ex:<br>SETIC,Setor de Informática e Comunicação"; ?>
                </label>
            </div>
...
<script>
    ...
    function saveRecord () {
        var jsonData = { 'tipo': $("#tipo").val(), 'id': $("#id").val(), 'descricao': $("#descricao").val(), 'sigla': $("#sigla").val() };
        $.post("<?= $this->url->getUrlController() ?>/save", jsonData, function(response) {
        ...
        if ($("#inFile").val().match(/\.csv/i)) { 
            ...
            $.ajax({
                ...
                if (response.result == 'success') {
                    ...
                    $.post("<?= $this->url->getUrlController() ?>/importarDadosDeArquivo", jsonData, function(response) {
                        if (response.result == 'success') {
                            $("#msg").addClass("alert-success");
                            $("#msg").fadeIn();
                            $("#msg").html(response.msg);
    ...
    $(document).ready(function() {
        recordTable = $("#simple-record-table").DataTable( {
            ...
            columns: [
                {"data": "id" },
                <?php  if ($tipo == 'setor') echo '{"data": "sigla" },'; ?>
                {"data": "descricao" },
                {
                    sortable: false,
                    render: function ( row, type, val, meta ) { 
                        return '<span class="btn btn-sm btn-warning glyphicon glyphicon-pencil" aria-hidden="true" onclick="editRecord('+val.id+',\''+val.descricao+'\',\''+val.sigla+'\')"></span> '+
